    ...
    def execute_cb(self, goal):
        ...
        if mode == 0 : 
            ...
            for i in range(10):
                ...
                self.casillas = self.detectarCasillas(frame)
        ...
        # Detect pieces        
            ...
            for color, (centro, _) in fichas.items():
                if centro:
                    ...
                    f.color = color
                    p = Pose()
                    p.position.x, p.position.y, p.position.z = self.piezaPixelToAruco(centro[0], centro[1], rvec, tvec)
                    p.orientation.x, p.orientation.y, p.orientation.z, p.orientation.w = 0, 0, 0, 1
                    cv2.circle(frame, centro, 10, colors[color], -1)
                    f.pose = p
                    f.idx = -1
                    if self.casillas:
                        idx = self.casillaPunto(centro, self.casillas)
                        cv2.putText(frame, color[0], centro, cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors[color], 2)
                        f.idx = idx
                    fichas_list.append(f)
            result.casillas = []
            result.fichas = fichas_list
            self.server.set_succeeded(result)


if __name__ == "__main__":
    nodo = TableroActionServer()
    rospy.spin()
