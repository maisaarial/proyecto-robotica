...
class TableroActionClient:
    ...
    def request_tablero(self, modo=1):
        ...
        goal = CasillasGoal()
        goal.modo = modo
        rospy.loginfo(f"[Casillas-CLIENT] Enviando solicitud de detección de tablero/fichas...")
        ...
        if modo == 0 :
            if result and result.casillas:
                rospy.loginfo(f"[Casillas-CLIENT] Castillas detectadas: {result.casillas}")
                tablero = result.casillas
            else:
                rospy.logwarn("[Casillas-CLIENT] No se detectaron casillas (tiempo de espera o aborto)")
            return tablero
        elif modo == 1 :
            if result and result.fichas:
                ...
                rospy.logwarn("[Casillas-CLIENT] No se detectaron fichas (tiempo de espera o aborto)")
            return tablero
